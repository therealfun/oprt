...
B<oprt-watch> B<always> shows if the current port needs a source update, as oposed to B<ck4up>, which shows B<only once> if the watched B<webpage has been modified>.
